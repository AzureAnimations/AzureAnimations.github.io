...
/* ──────────────────────────────────────────────────────────────────────────
   "Tap to show love" cue — deliberately OUTSIDE the block above.
   That block returns early when the Supabase CDN client is missing, which would
   silently take this cue down with it on a slow/blocked network. The cue needs
   no backend, so it installs on its own.
   ────────────────────────────────────────────────────────────────────────── */
(function () {
  'use strict';
  if (window.__awLikeNudgeLoaded) return;
  window.__awLikeNudgeLoaded = true;
  ...
  function alreadyLiked(top) {
    var btn = top.querySelector('[data-aw-like]');
    if (!btn) return false;
    if (btn.classList.contains('is-liked')) return true;
    // the class is only applied once the likes client boots, so fall back to its storage key
    try { return localStorage.getItem('aw_liked_' + (btn.getAttribute('data-page-slug') || location.pathname)) === '1'; }
    catch (e) { return false; }
  }

  function paint() {
    var tops = document.querySelectorAll('.aw-like-top');
    for (var i = 0; i < tops.length; i++) {
      var top = tops[i];
      var n = top.querySelector('.like-nudge');
      if (!n) {
        n = document.createElement('span');
        n.className = 'like-nudge';
        n.setAttribute('aria-hidden', 'true');
        top.appendChild(n);   // after the button, so `.is-liked ~ .like-nudge` can retire it
      }
      n.textContent = text();
      n.style.display = alreadyLiked(top) ? 'none' : '';
    }
  }
