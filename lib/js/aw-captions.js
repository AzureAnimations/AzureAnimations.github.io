/* ──────────────────────────────────────────────────────────────────────────
   ...
   Per-page wiring (self-contained pages):
     AWCaptions.configure({
       ...
       base:     '../narration/Storage/',      // dir holding <page>.md (+ <lang>/)
...
(function () {
	...
	function splitSentences(text) {
		return text.split(SENT_RE).map(function (s) { return s.trim(); }).filter(Boolean);
	}
	...
	function loadSteps() {
		...
		// no-store: a cached script would caption the wrong step after the narration is re-cut
		return fetch(narrationUrl(), { cache: 'no-store' })
	...
	// Speaking time = clip length minus the scripted pauses; give each segment speech time by its
	// length, then hold it through its trailing pause. Shared by the caption bar and track().
	function makeIndexer(list) {
		const lens = list.map(function (o) { return (o.t || '').length; });
		...
		return function (cur, dur) {
			const speechDur = Math.max(dur - totalPause, dur * 0.4);
			let idx = 0, tAcc = 0;
	...
	function bind(audio, stepIdx) {
		...
		loadSteps().then(function (steps) {
			...
			const indexAt = makeIndexer(list);
			curSeg = -1;
			show(0);
			onTime = function () {
				...
				const idx = indexAt(audio.currentTime, d);
	...
	// Which SENTENCE of the script is being spoken right now, without rendering anything. Lets a page
	// react to the narration (spotlight an element, advance a diagram) even with captions switched
	// off — bind() deliberately no-ops when they are. Returns a stop() function.
	// cb(index, total, text). Match on `text`, not `index`: parse() splits lines into sentences, so
	// the index shifts with punctuation and is NOT the same across locales.
	function track(audio, stepIdx, cb) {
		if (!audio || typeof cb !== 'function') return function () { };
		let stopped = false, onT = null, last = -1;
		loadSteps().then(function (steps) {
			if (stopped) return;
			const list = (steps && steps[stepIdx]) || [];
			...
			onT = function () {
				...
				if (!isFinite(d) || d <= 0) return;
				const idx = indexAt(audio.currentTime, d);
				if (idx !== last) { last = idx; cb(idx, list.length, (list[idx] || {}).t || ''); }
			};
			audio.addEventListener('timeupdate', onT);
		});
		return function () { stopped = true; if (onT) audio.removeEventListener('timeupdate', onT); };
	}
