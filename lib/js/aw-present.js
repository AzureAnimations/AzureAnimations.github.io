...
(function () {
	...
	// presenter button lifecycle: idle -> ready (audience open) -> live (playing)
	var presPhase = 'idle';
	var audienceWin = null;    // presenter: handle from window.open, so we can close it
	var audienceWatch = null;
	...
	// One-shot token written by the presenter when it chains to the next journey
	// stop, so the audience window resumes instantly instead of re-showing the
	// "waiting for the presenter" overlay.
	var HANDOFF = 'aw-present-handoff';
	...
	// Presenter side: the main button walks Present -> Start -> Close, and the
	// secondary button is Pause / Resume once the show is running.
	function updatePlayBtn() {
		var main = document.getElementById('present-btn');
		if (main) {
			var mIco = main.querySelector('.aw-present-ico');
			var mLab = main.querySelector('.aw-present-label');
			main.classList.remove('is-ready', 'is-live');
			if (presPhase === 'idle') {
				if (mIco) mIco.textContent = '\uD83D\uDDA5\uFE0F';
				if (mLab) mLab.textContent = 'Click to present';
				main.setAttribute('aria-label', 'Open presenter display in a new window');
				main.title = 'Open presenter display (new window) \u2014 drag to your 2nd monitor';
			} else if (presPhase === 'ready') {
				main.classList.add('is-ready');
				if (mIco) mIco.textContent = '\u25B6';
				if (mLab) mLab.textContent = 'Start';
				main.setAttribute('aria-label', 'Start the animation in the audience window');
				main.title = 'Start the animation in the audience window';
			} else {
				main.classList.add('is-live');
				if (mIco) mIco.textContent = '\u2715';
				if (mLab) mLab.textContent = 'Close';
				main.setAttribute('aria-label', 'End the presentation and close the audience window');
				main.title = 'End the presentation and close the audience window';
			}
		}
		var b = document.getElementById('present-play');
		if (!b) return;
		b.classList.toggle('is-visible', presPhase === 'live');
		var ico = b.querySelector('.aw-play-ico');
		var lab = b.querySelector('.aw-play-label');
		if (pausedState) { if (ico) ico.textContent = '\u25B6'; if (lab) lab.textContent = 'Resume'; b.setAttribute('aria-label', 'Resume the animation'); }
	...
	function beginPlayback() {
		...
		pausedState = false;
		presPhase = 'live';
		sendControl('start', lastStep);
		sendControl('speed', null, currentSpeed());
		sendControl('theme', null, currentTheme());
		updatePlayBtn();
	}
	function onPresentClick() {
		if (presPhase === 'idle') openAudience();
		else if (presPhase === 'ready') beginPlayback();
		else closeAudience();
	}
	function onPlayClick() {
		if (presPhase !== 'live') return;
		if (!pausedState) { setPaused(true); sendControl('pause'); }
	...
	function injectStyle() {
		...
		var css = ''
			...
			+ '#present-btn.is-ready{color:#fff;background:#0a84ff;animation:none;}'
			+ '#present-btn.is-live{color:#d13438;border-color:#d13438;background:transparent;animation:none;}'
			+ '#present-btn.is-live:hover{color:#fff;background:#d13438;box-shadow:0 3px 12px rgba(209,52,56,.35);}'
			+ '@keyframes awPresentPulse{0%,100%{box-shadow:0 0 0 0 rgba(10,132,255,0);}50%{box-shadow:0 0 0 4px rgba(10,132,255,.16);}}'
			+ 'body.present-mode .app-header,body.present-mode .progress-track,body.present-mode #controls,body.present-mode #header-reveal,body.present-mode #step-outline,body.present-mode .stage-overlay,body.present-mode .stage-overlay-left,body.present-mode #shortcut-overlay,body.present-mode .aw-drag-reset,body.present-mode #aw-play-card,body.present-mode #journey-next{display:none !important;}'
			+ 'body.present-mode{padding-bottom:0 !important;overflow:hidden !important;}'
			// Present mode is shown fullscreen, so the framing rectangle (border,
			// gradient ring, rounded corners, drop shadow) is just noise — the slide
			// itself is the frame.
			+ 'body.present-mode #stage-frame{border:none !important;border-radius:0 !important;background:transparent !important;box-shadow:none !important;}'
			+ 'body.present-mode #stage-frame::before,body.present-mode #stage-frame::after{display:none !important;content:none !important;}'
	...
	function injectButton() {
		...
		// Pause / Resume for the running show (the main button owns start & close).
		var play = document.createElement('button');
		play.id = 'present-play';
		play.type = 'button';
		play.setAttribute('aria-label', 'Pause the animation');
		play.innerHTML = '<span class="aw-play-ico" aria-hidden="true">\u23F8</span><span class="aw-play-label">Pause</span>';
		...
		btn.addEventListener('click', function (e) { e.stopPropagation(); onPresentClick(); });
	}
	...
	function openAudience() {
		...
		function spawn() { audienceWin = window.open(url, 'aw-present-audience', feat); }
		if (audienceAlive()) {
		...
		broadcast(lastStep);
		// Fresh audience window: offer Start next, and Pause only once it is running.
		presStarted = false;
		pausedState = false;
		presPhase = 'ready';
		watchAudience();
	...
	// End the show: close the audience window and put the button back to Present.
	// The message covers a window we have no handle for (one reused across journeys).
	function closeAudience() {
		gSend({ type: 'close-audience' });
		try { if (audienceWin && !audienceWin.closed) audienceWin.close(); } catch (_) {}
		audienceWin = null;
		try { localStorage.removeItem(ALIVE); } catch (_) {}
		if (audienceWatch) { clearInterval(audienceWatch); audienceWatch = null; }
		presStarted = false;
		pausedState = false;
		presPhase = 'idle';
		updatePlayBtn();
	...
	// If the presenter closes the audience window by hand, fall back to Present.
	function watchAudience() {
		if (audienceWatch) clearInterval(audienceWatch);
		audienceWatch = setInterval(function () {
			if (!audienceWin) return;          // reused window: no handle to poll
			if (audienceWin.closed) closeAudience();
		}, 1000);
	}

	// ── "Up next" hand-off (presenter side) ─────────────────────────────────
	// The last step of every journey offers a pill that chains to the next stop.
	// Following that link used to strand the audience window on the old page, so
	// during a live show we send it ahead (with ?present) and leave a hand-off
	// token telling it to resume without the "waiting" overlay.
	// Point the audience window at a slide, tagging it as a hand-off so that window resumes the
	// show instead of dropping back to the "waiting for the presenter" overlay.
	function sendAudienceTo(url, tag) {
		try { localStorage.setItem(HANDOFF, JSON.stringify({ path: url.pathname, n: Date.now() })); } catch (_) {}
		gSend({ type: 'navigate', url: url.pathname + url.search, nonce: tag + '-' + Date.now() });
	}
	function handOff(href) {
		var next = new URL(href, location.href);
		// Journeys end by linking back to a hub, but hubs are ordinary pages — they have no
		// slideshow and don't load this script, so sending the audience there would swap the
		// slide for the full site chrome (header, hero, footer). Leave that window on the closing
		// slide; boot() pulls it across as soon as the presenter opens the next journey.
		if (/(^|\/)index\.html$/i.test(next.pathname) || next.pathname.slice(-1) === '/') {
			location.href = next.href;
			return;
		}
		var aud = new URL(next.href);
		aud.searchParams.set('present', '');
		sendAudienceTo(aud, 'handoff');
		// Let the audience window pick the message up before this one unloads.
		setTimeout(function () { location.href = next.href; }, 150);
	}
	function wireJourneyNext() {
		var cta = document.getElementById('jn-cta');
		if (!cta || cta.__awPresent) return;
		cta.__awPresent = true;
		cta.addEventListener('click', function (e) {
			if (isPresent) { e.preventDefault(); return; }
			if (presPhase !== 'live' || !audienceAlive()) return;   // no live show — plain link
	...
	function initPresent() {
		...
		// Did the presenter chain here from the previous stop? Then the show is
		// already running and this window plays straight away.
		var handoff = false;
		try {
			var raw = localStorage.getItem(HANDOFF);
			if (raw) {
				var h = JSON.parse(raw);
				if (h && h.path === location.pathname && (Date.now() - (h.n || 0)) < 30000) {
					handoff = true;
		...
		gOn(function (msg) {
			...
			// Compare paths, not whole URLs: the sender carries ?lang/?step/?present, so a string
			// match would treat "the page you are already on" as a new destination and reload it,
			// restarting the slide every time the presenter refreshes.
			var target = msg.url.split('#')[0].split('?')[0];
			if (target !== location.pathname) location.href = msg.url;
		});

		// The presenter pressed Close.
		gOn(function (msg) {
			if (!msg || msg.type !== 'close-audience') return;
			clearBeat();
	...
	function boot() {
		...
		// way to drive it — come back with Close (and Pause) already available.
		// This is also how a hand-off resumes: the new page finds the audience
		// still alive and picks the show back up mid-flight.
		if (!isPresent && audienceAlive()) {
			presStarted = true;
			presPhase = 'live';
			watchAudience();
			sendControl('theme', null, currentTheme());
			// That window may still be showing the previous journey's closing slide (a journey ends
			// on a hub, which we never send it to), so bring it to this page. Same-page navigations
			// are ignored by the receiver, so a plain presenter reload costs nothing.
			var hereUrl = new URL(location.href);
			hereUrl.searchParams.set('present', '');
			sendAudienceTo(hereUrl, 'resume');
		}
		if (!isPresent) updatePlayBtn();
		if (isPresent) {
			initPresent();
