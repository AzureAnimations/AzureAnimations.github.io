...
(function () {
	...
	function injectStyle() {
		...
		var css = ''
			...
			// "Up next" on the audience screen is a signpost, not a control — the
			// presenter drives the hand-off from the main window.
			+ 'body.present-mode #journey-next{pointer-events:none;}'
			+ 'body.present-mode #jn-cta,body.present-mode #jn-close{display:none !important;}'
			+ 'body.present-mode .jn-card{padding-right:1rem;}'
	...
	function handOff(href) {
		...
		try { localStorage.setItem(HANDOFF, JSON.stringify({ path: aud.pathname, n: Date.now() })); } catch (_) {}
		gSend({ type: 'navigate', url: aud.pathname + aud.search, nonce: 'handoff-' + Date.now() });
