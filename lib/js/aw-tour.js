...
(function () {
	...
	function clipSrc(i) {
		...
		var lang = curLang();
		var src = cfg.audioBase + (lang && lang !== 'en' ? lang + '/' : '') + 'step-' + pad2(i + 1) + '.mp3';
		return src + (cfg.audioVersion ? '?v=' + encodeURIComponent(cfg.audioVersion) : '');
	}
	...
	function injectCss() {
		...
		s.textContent = [
			...
			'body.awt-on .topbar, body.awt-on .site-head, body.awt-on .bar, body.awt-on .aw-like-pop, body.awt-on .like-nudge{opacity:0.18 !important;pointer-events:none !important;}',
	...
	function place(target) {
		...
		var r = target.getBoundingClientRect(), left, stacked = false;
		if (r.right + GAP + w <= innerWidth - EDGE) left = r.right + GAP;        // prefer right
		else if (r.left - GAP - w >= EDGE) left = r.left - GAP - w;              // else left
		else {                                                                   // narrow screen: no room beside it
			left = Math.min(Math.max(r.left + (r.width - w) / 2, EDGE), innerWidth - w - EDGE);
			stacked = true;
		}
		var top;
		if (stacked) {
			// Centring here would park the card on top of the node it is pointing at, so sit
			// under it when there is room in the band, otherwise above it.
			top = r.bottom + GAP <= hiT ? r.bottom + GAP
				: (r.top - GAP - h >= loT ? r.top - GAP - h : (r.top - GAP - h > hiT - h ? loT : hiT));
		} else {
			top = r.top + (r.height - h) / 2;
		}
		top = Math.min(Math.max(top, loT), hiT);
	...
	function go(n) {
		...
		function show() {
			...
			card.innerHTML =
				...
				(href ? '<a class="awt-open" href="' + href + '">' + (st.openLabel || cfg.openLabel || 'Open this animation \u2192') + '</a>' : '');
			...
			if (src) {
				...
				if (!paused) {
					var p = audio.play();
					if (p && p.catch) p.catch(function () { armFallback(); });
				}
