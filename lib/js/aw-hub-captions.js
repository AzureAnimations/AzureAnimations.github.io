/* ──────────────────────────────────────────────────────────────────────────
   ...
   Drop-in: add ONE line to a hub <head> and it wires everything —
       <script src="aw-hub-captions.js"></script>     (one-level hubs)
   ...
   ../…/narration/<domain>/index.md (+ <lang>/). No per-hub JS edits needed.
   ────────────────────────────────────────────────────────────────────────── */
(function () {
	'use strict';
	...
	// This file always sits at <siteRoot>/lib/js/, so its own URL is the only reliable
	// anchor for the site root — works at any base path, unlike counting path depth.
	//   /journeys/Security/index.html   → <root>/narration/Security/
	//   /journeys/AI/Foundry/index.html → <root>/narration/AI/Foundry/
	//   /index.html                     → <root>/narration/
	var JOURNEYS = 'journeys/';           // pages nest under this, narration/ stays flat
	function narrationBase() {
		var m = ((MY && MY.src) || '').match(/^(.*\/)lib\/js\/aw-hub-captions\.js(?:[?#].*)?$/);
		if (!m) return '../../narration/';
		var root = m[1];
		var rootPath = new URL(root, location.href).pathname;
		var dir = location.pathname.replace(/[^/]*$/, '');
		var rel = dir.indexOf(rootPath) === 0 ? dir.slice(rootPath.length) : '';
		if (rel.indexOf(JOURNEYS) === 0) rel = rel.slice(JOURNEYS.length);
		return root + 'narration/' + rel;
	}
